...
/**
 * Xinference请求转换器
 * 将标准请求转换为Xinference API格式
 */
@Slf4j
@RequiredArgsConstructor
public class XinferenceRequestTransformer {
    ...
    private Object transformChatRequest(final ChatDTO.Request request) {
        try {
            ...
            if (request.temperature() != null) xinferenceRequest.put("temperature", request.temperature());
            if (request.maxTokens() != null) xinferenceRequest.put("max_tokens", request.maxTokens());
            if (request.topP() != null) xinferenceRequest.put("top_p", request.topP());
            if (request.stop() != null) xinferenceRequest.set("stop", objectMapper.valueToTree(request.stop()));
            ...
            log.warn("Failed to transform Xinference chat request: {}", e.getMessage());
            return request;
        }
    }

    private ObjectNode buildChatExtraBody(final ChatDTO.Request request) {
    ...
    private Object transformEmbeddingRequest(final EmbeddingDTO.Request request) {
        try {
            ...
            if (request.encodingFormat() != null) xinferenceRequest.put("encoding_format", request.encodingFormat());
            if (request.dimensions() != null) xinferenceRequest.put("dimensions", request.dimensions());
            if (request.user() != null) xinferenceRequest.put("user", request.user());
            ...
            log.warn("Failed to transform Xinference embedding request: {}", e.getMessage());
            return request;
        }
    }
    ...
    private Object transformRerankRequest(final RerankDTO.Request rerankRequest) {
        try {
            ...
            xinferenceRequest.put("model", rerankRequest.model());
            ...
            log.warn("Failed to transform Xinference rerank request: {}", e.getMessage());
    ...
    private Object transformTtsRequest(final TtsDTO.Request ttsRequest) {
        try {
            ...
            xinferenceRequest.put("model", ttsRequest.model());
            ...
            log.warn("Failed to transform Xinference TTS request: {}", e.getMessage());
            return ttsRequest;
        }
    }
    ...
    private Object transformSttRequest(final SttDTO.Request sttRequest) {
        try {
            ...
            log.warn("Failed to transform Xinference STT request: {}", e.getMessage());
            return sttRequest;
        }
    }
}
