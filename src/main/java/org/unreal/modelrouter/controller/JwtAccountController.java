...
public class JwtAccountController {
    ...
    public Mono<ResponseEntity<RouterResponse<List<JwtAccountResponse>>>> getAllAccounts() {
        log.debug("获取所有JWT账户");
        ...
        return accountConfigurationService.getAllAccounts()
                .map(accounts -> accounts.stream()
                        ...
                        .toList())
                .map(data -> ResponseEntity.ok(RouterResponse.success(data, "成功获取JWT账户列表")))
                .doOnSuccess(result -> log.debug("成功获取 {} 个JWT账户", result.getBody().getData().size()))
    ...
    public Mono<ResponseEntity<RouterResponse<JwtAccountResponse>>> getAccountByUsername(
        ...
        return accountConfigurationService.getAccountByUsername(username)
                .map(account -> {
                    if (account == null) {
                        return ResponseEntity.notFound().<RouterResponse<JwtAccountResponse>>build();
                    }
                    return ResponseEntity.ok(RouterResponse.success(jwtUserProperties.convertToResponse(account), "成功获取JWT账户"));
                })
                .doOnSuccess(result -> {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> createAccount(
        ...
        return accountConfigurationService.createAccount(jwtUserProperties.convertToAccount(request))
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户创建成功: " + request.getUsername()))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> updateAccount(
            @Parameter(description = "用户名", required = true)
            @PathVariable @NotBlank String username,
        ...
        return accountConfigurationService.updateAccount(username, jwtUserProperties.convertToAccount(request))
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户更新成功: " + username))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> deleteAccount(
            @Parameter(description = "用户名", required = true)
            @PathVariable @NotBlank String username) {
        log.info("删除JWT账户: {}", username);
        
        return accountConfigurationService.deleteAccount(username)
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户删除成功: " + username))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> setAccountEnabled(
            @Parameter(description = "用户名", required = true)
            @PathVariable @NotBlank String username,
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> batchUpdateAccounts(
        ...
        return accountConfigurationService.batchUpdateAccounts(accounts)
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户批量更新成功，数量: " + requests.size()))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<List<Integer>>>> getAllAccountVersions() {
        log.debug("获取JWT账户配置版本列表");
        
        return Mono.fromCallable(() -> accountConfigurationService.getAllAccountVersions())
                .map(data -> ResponseEntity.ok(RouterResponse.success(data, "成功获取JWT账户配置版本列表")))
                .doOnSuccess(result -> log.debug("成功获取 {} 个JWT账户配置版本", result.getBody().getData().size()))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Map<String, Object>>>> getAccountVersionConfig(
        ...
        return Mono.fromCallable(() -> accountConfigurationService.getAccountVersionConfig(version))
                .map(config -> {
                    if (config == null) {
                        return ResponseEntity.notFound().<RouterResponse<Map<String, Object>>>build();
                    }
                    return ResponseEntity.ok(RouterResponse.success(config, "成功获取JWT账户配置版本"));
                })
                .doOnSuccess(result -> {
                    if (result.getStatusCode().is2xxSuccessful()) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> applyAccountVersion(
            @Parameter(description = "版本号", required = true)
            @PathVariable int version) {
        log.info("应用JWT账户配置版本: {}", version);
        
        return Mono.fromRunnable(() -> accountConfigurationService.applyAccountVersion(version))
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户配置版本应用成功: " + version))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Integer>>> getCurrentAccountVersion() {
        log.debug("获取当前JWT账户配置版本号");
        
        return Mono.fromCallable(() -> accountConfigurationService.getCurrentAccountVersion())
                .map(data -> ResponseEntity.ok(RouterResponse.success(data, "成功获取当前JWT账户配置版本号")))
                .doOnSuccess(result -> log.debug("当前JWT账户配置版本号: {}", result.getBody().getData()))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<Void>>> resetAccountsToDefault() {
        log.info("重置JWT账户配置为默认值");
        
        return accountConfigurationService.resetAccountsToDefault()
                .then(Mono.just(ResponseEntity.ok(RouterResponse.success("JWT账户配置已重置为默认值"))))
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<RouterResponse<JwtAccountConfigStatus>>> getAccountConfigStatus() {
        ...
        .map(data -> ResponseEntity.ok(RouterResponse.success(data, "成功获取JWT账户配置状态")))
