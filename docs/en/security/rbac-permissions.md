...
1. **Permission code**: 48 `module:resource:action` codes (`action` ∈ `read` / `write` / `manage`) drive both backend URL access decisions and frontend menu/route visibility.
...
- 48 codes in total: the config module contributes 12 resources × read/write = 24 codes; the remaining modules are shown above.
...
|------|:----------------:|------------------|-------|
| ADMIN | 48 | All permission codes | Superset; bypasses URL rules, and full codes are embedded at JWT issuance |
| OPERATOR | 39 | All `:read` + `:write` codes | Excludes `system:*`, `security:*:manage`, `actuator:*`; keeps `security:audit:read`, no `callhistory:view` |
| USER | 27 | Dashboard + config read + full lb/cb/rl + monitoring read + tracing dashboard/search + AI playground | Read-oriented; only write code is `lb:config:write`; no call history view |
...
|--------|------|-------------|---------------------|
| GET | `/api/security/permissions` | All permission codes (48, in catalog order) | `system:permissions:manage` |
...
| GET | `/api/auth/permissions` | Permission codes of the current user (ADMIN gets the full 48) | Any authenticated user |
...
The `data` field of the response is an array of the 48 permission codes (same order as the full catalog).
...
ADMIN receives the full 48 codes; other roles receive the codes from their JWT `permissions` claim.
...
2. **Permission code tree**: 48 codes grouped by module as a checkable tree (leaf nodes are permission codes; parent nodes toggle automatically).
