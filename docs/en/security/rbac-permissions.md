...
1. **Permission code**: 45 `module:resource:action` codes (`action` ∈ `read` / `write` / `manage`) drive both backend URL access decisions and frontend menu/route visibility.
...
- 45 codes in total: the config module contributes 11 resources × read/write = 22 codes; the remaining modules are shown above.
...
|------|:----------------:|------------------|-------|
| ADMIN | 45 | All permission codes | Superset; bypasses URL rules, and full codes are embedded at JWT issuance |
| OPERATOR | 36 | All `:read` + `:write` codes | Excludes `system:*`, `security:*:manage`, `actuator:*`; keeps `security:audit:read`, no `callhistory:view` |
| USER | 25 | Dashboard + config read + full lb/cb/rl + monitoring read + tracing dashboard/search + AI playground | Read-oriented; only write code is `lb:config:write`; no call history view |
| VIEWER | 24 | All `:read` codes | Pure read-only role; no `callhistory:view`, `ai:playground:use`, or other non-`:read` codes |
...
The `data` field of the response is an array of the 45 permission codes (same order as the full catalog).
...
ADMIN receives the full 45 codes; other roles receive the codes from their JWT `permissions` claim.
...
2. **Permission code tree**: 45 codes grouped by module as a checkable tree (leaf nodes are permission codes; parent nodes toggle automatically).
