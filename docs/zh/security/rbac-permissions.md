...
1. **权限码**：45 个 `module:resource:action` 权限码（`action` ∈ `read` / `write` / `manage`），同时驱动后端 URL 访问判定与前端菜单/路由可见性。
...
- 合计 45 个：config 模块 11 个资源全部 read/write 成对 = 22 码，其余模块见上表。
...
| 角色 | 权限码数 | 权限范围 | 说明 |
|------|:-------:|----------|------|
| ADMIN | 45 | 全量权限码 | 超集；URL 规则直通，JWT 签发时内嵌全量码 |
| OPERATOR | 36 | 全部 `:read` + `:write` 码 | 排除 `system:*`、`security:*:manage`、`actuator:*`；保留 `security:audit:read`，不含 `callhistory:view` |
| USER | 25 | 仪表盘 + config 只读 + lb/cb/rl 全量 + monitoring 只读 + tracing dashboard/search + AI 试验场 | 只读为主，唯一写码 `lb:config:write`；不含调用历史查看 |
| VIEWER | 24 | 全部 `:read` 权限码 | 纯只读角色；不含 `callhistory:view`、`ai:playground:use` 等非 `:read` 码 |
...
| 方法 | 路径 | 说明 | 所需权限 |
|------|------|------|----------|
| GET | `/api/security/permissions` | 全部权限码（45 个，按清单顺序） | `system:permissions:manage` |
...
| GET | `/api/auth/permissions` | 当前登录用户权限码（ADMIN 返回全量 45 码） | 任意已登录用户 |
...
ADMIN 直接返回全量 45 码；其他角色返回其 JWT `permissions` claim 中的权限码。
...
2. **权限码树**：45 个权限码按模块分组的树形勾选（叶子为权限码，父节点自动联动）。
