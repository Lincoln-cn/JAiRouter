...
JAiRouter 支持 JWT 账户从 YAML 配置自动初始化到数据库。系统启动时会自动将配置中的账户同步到数据库。
...
- **自动同步**：系统启动时自动将 YAML 配置中的账户写入数据库
- **重复处理**：已存在的账户不会被重复创建
- **密码编码**：支持 `{bcrypt}` 和 `{noop}` 密码编码器
- **角色分配**：账户创建时自动分配配置的角色

### 账户状态管理 (v1.7.0+)

支持账户启用/禁用状态切换，便于临时禁用可疑账户。

#### 账户状态切换 API

```bash
# 切换账户状态（启用/禁用）
curl -X POST "http://localhost:8080/api/security/jwt/accounts/{accountId}/toggle-status" \
     -H "Authorization: Bearer admin_token"
```
...
| 状态 | 说明 |
|------|-----|
| `enabled: true` | 账户正常可用，可以登录和访问 API |
| `enabled: false` | 账户已禁用，无法登录，已有令牌继续有效直到过期 |
...
```bash
curl -X GET "http://localhost:8080/api/security/jwt/accounts?page=0&size=20" \
     -H "Authorization: Bearer admin_token"
```
...
```bash
curl -X PUT "http://localhost:8080/api/security/jwt/accounts/{accountId}" \
     -H "Authorization: Bearer admin_token" \
     -H "Content-Type: application/json" \
     -d '{
...
```bash
curl -X DELETE "http://localhost:8080/api/security/jwt/accounts/{accountId}" \
     -H "Authorization: Bearer admin_token"
```

## 安全黑名单 (v1.7.0+)

JAiRouter 新增安全黑名单管理功能，支持 IP/用户/令牌级别的黑名单。

### 黑名单类型

| 类型 | 说明 | 使用场景 |
|------|-----|---------|
| IP 黑名单 | 阻止特定 IP 地址访问 | 防止恶意 IP 攻击 |
| 用户黑名单 | 阻止特定用户登录 | 禁用可疑用户 |
| 令牌黑名单 | 阻止特定令牌使用 | 撤销被盗令牌 |
...
```bash
curl -X GET "http://localhost:8080/api/security/blacklist?type=IP&page=0&size=20" \
     -H "Authorization: Bearer admin_token"
```
...
```bash
curl -X DELETE "http://localhost:8080/api/security/blacklist/{entryId}" \
     -H "Authorization: Bearer admin_token"
```
...
```bash
curl -X GET "http://localhost:8080/api/security/blacklist/stats" \
     -H "Authorization: Bearer admin_token"
```
