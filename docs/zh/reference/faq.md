...
### Q12.1: Web 管理台已支持中英双语，为什么有些提示仍是中文？

**A**: 自 v2.10.3（Web 双语版）起，Web 管理台界面已全量中英双语化，并可在页面头部切换语言。当前的语言策略如下：

- **前端界面文案**：登录页、菜单、按钮、对话框等 UI 文案均通过 i18n 本地化，随界面语言即时切换，并为通用兜底场景（如网络异常）提供对应语言的提示；
- **后端直返消息**：由后端 API 直接返回的 `message` 字段（如错误与校验消息）按既定策略**保持原文透传、不随界面语言翻译**——即使界面为英文，此类消息仍为中文原文；
- **关键操作提示**：对部分常见后端错误（如版本不存在、配置损坏、权限不足、系统错误），前端已在本地分类并映射为当前语言的展示文案。

如希望后端消息也随界面语言变化，属于后端错误码化改造范畴，需另行立项，不在当前版本支持。
