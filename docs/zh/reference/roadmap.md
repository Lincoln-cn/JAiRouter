...
### ✅ v3.2.2（补丁：用户上报问题修复 + 管理面 HTTP 状态码语义，已发布 2026-09-22）

**状态**: 已发布（Git tag / GitHub Release `v3.2.2`；Docker 镜像随 Release 由 CI 重建）
**范围**: 用户上报的 #83 / #84 修复，并连带收口相邻缺陷（#92 / #94 / #95 / #96 / #98）；纯缺陷修复，无新功能（PR #90 / #91 / #93 / #97 / #99）

| 领域 | 交付 |
|------|------|
| API Key 配额 | 创建/编辑 API Key 补全配额字段提交，修复创建时设置的配额在前端回显异常（#84）；配额更新/重置接口去掉 Reactor 非阻塞线程上的 `block()`（原本 100% 失败，#92）；告警阈值三处口径统一为 `[0.05,1]` 闭区间，新增 `QuotaLimits` 单一事实来源并为创建路径补上校验（#96） |
| 负载均衡与路由监控 | 一条请求内实例重复选择 3 次收敛为 1 次（请求级 `SelectedInstanceHolder` + 适配器复用已选实例），修复「实例分布」数字跳变 +3（#83）；连带修复适配器侧未透传请求头致 header 规则/标签路由失效、负载均衡 `recordCall` 每请求推进 2 次 |
| 管理面 HTTP 状态码 | 新增 `ApiException` / `ApiExceptions`，138 处「失败伪装为 200」站点全部迁移为正确 4xx/5xx（**响应体逐字兼容**，#94）；前端拦截器透出后端错误消息；e2e 断言收紧（#95）；`MonitoringController` 客户端校验错误码统一为 400 并补自动模式测试（#98） |
| 工程与工具 | 新增 OpenAI 兼容 mock 模型服务；`.gitignore` 忽略本地 `docker-ctx` |
...
| v3.2.2 | 2026-09-22 | 用户上报问题补丁：API Key 配额回显（#84）与路由监控计数 +3（#83）修复 + 配额接口 `block` 失败（#92）+ 管理面业务失败由 HTTP 200 改为 4xx/5xx（#94，响应体兼容）+ e2e 断言收紧（#95）+ 告警阈值统一 `[0.05,1]`（#96）+ 监控接口错误码统一（#98）（PR #90/#91/#93/#97/#99） |
...
- ✅ 请求/响应数据脱敏
- ✅ 安全审计日志
- ✅ H2 嵌入式数据库（当前唯一受支持的持久化数据库）
- 📋 PostgreSQL/MySQL 支持（规划中；当前 `pom.xml` 仅引入 H2 驱动，配置与方言均硬编码为 H2，尚无外部数据库适配）
- ✅ Redis 缓存集成
- ✅ Prometheus 指标收集
- ✅ Grafana 仪表板模板
...
#### 数据存储
- **默认**: H2 嵌入式数据库
- **当前唯一支持**: H2 嵌入式单文件数据库（外部共享数据库如 PostgreSQL / MySQL 规划中，见 issue #160）
- **缓存**: Redis (可选)
...
- **兼容性**: 多版本 API 兼容性维护
- **安全性**: 安全漏洞和攻击防护
- **横向扩展受限**: 当前唯一支持 H2 嵌入式单文件数据库，多副本无法共享同一数据文件，`replicas` 必须为 1；限流、配额、JWT 黑名单、API-Key 缓存等状态默认均为进程内语义。落地顺序见 issue #160（共享数据库适配）、#161（限流跨实例化）、#162（共享态接线）、#163（调度锁）、#164（实时事件广播）、#165（K8s 制品）
...
11. ✅ v3.2.2 补丁（2026-09-22 发布）：用户上报的 API Key 配额回显（#84）与路由监控计数 +3（#83）修复；管理面业务失败由 HTTP 200 改为 4xx/5xx（#94，响应体兼容），并收口配额接口 `block` 失败（#92）、e2e 断言假通过（#95）、告警阈值口径（#96）、监控接口错误码（#98）。**已知剩余（P2）**：`QuotaLedgerService` 分布式路径同步 `block`、前端临时 ID 使用 `Math.random`
12. 📋 v3.3.0 候选：脱敏规则持久化版本化、配额 HOUR/MONTH 限额、记录级 FULL 加密策略 UI、试脱敏回归集
13. 📋 语义缓存评估（向量相似度复用，独立项目）；高可用地基（多节点/Redis）在 v3.2.x 后再评估
