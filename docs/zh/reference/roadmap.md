...
#### 主要功能（v2.9.6 → v3.0.1 演进）
| 版本 | 日期 | 核心内容 |
|------|------|---------|
| v2.9.6 | 2026-08-30 | 路由智能深化-2：请求级故障转移（换实例重试 + 请求内黑名单 + 全失败回退） |
| v2.9.7 | 2026-09-01 | 路由智能深化-3：标签路由（实例 tags + TARGET_TAGS 动作 + header 圈选 + 旧库迁移/限流器修复） |
| v2.9.8 | 2026-09-02 | Web 菜单 RBAC 管理（43 权限码 + 4 角色模板 + 数据驱动 URL 权限矩阵 + 权限管理 UI + 登录 DB fallback） |
| v2.9.9 | 2026-09-03 | 路由智能深化-4：响应缓存 P0（非流式精确匹配缓存 + Caffeine + 租户隔离键 + 指标） |
| v2.9.10 | 2026-09-05 | 路由智能深化-5：响应缓存 P1（流式 SSE 拼接缓存 + 失效 API + 缓存键三段式 + 服务级限流提前短路 + 权限码 44） |
| v2.9.11 | 2026-09-05 | 文档与 UI 品质：README 截图/内容更新（亮/暗 + 权限码 44 + 响应缓存 P1）+ hallmark 管理台设计审计（v2.10.x 重构输入）+ 前端小修（权限码 44 同步等） |
| v2.10.0 | 2026-09-05 | Web 前端地基：死代码清理（-13.5k 行）+ PageSkeleton/StatCard/useChartTheme 统一组件 + ~20 页令牌化（~150 hex→token）+ Element Plus 暗色修复 + Layout/console 清理 |
| v2.10.1 | 2026-09-06 | 治理指挥台：Dashboard v2（治理链路面板/异常摘要/服务跳转）+ Cross-link 跨页导航 + 7 页 PageSkeleton 迁移 + 令牌化收尾（~102 hex→token + 间距梯度） |
| v2.10.2 | 2026-09-06 | 配置接入与能力补页：响应缓存管理页（状态/定向失效 + B 档运行时开关 + 命中观测）+ 慢查询分析页 + CB 重置/配额告警 + Onboarding 接入引导 + 后端缓存状态与废弃标注（权限码 45） |
| v2.10.3 | 2026-09-06 | Web 双语版：全量 zh/en i18n（39 语言包/2321 keys）+ 语言切换器 + Element Plus locale + 日期/数字本地化 + 暗色对比收口 |
| v2.10.4 | 2026-09-06 | Web 体验收尾：服务类型显示名统一（顶层 serviceTypes.*）+ ECharts 语言/主题热切换 + 死代码清理 + 暗色对比审计门禁（36 页 ≥4.5）+ 后端 message 注记 + 代码块双主题高亮 |
| v3.0.1 | 2026-09-06 | Web 完整流程系列开篇（流程串联）：Onboarding 预选闭环 + Dashboard 治理入口补全 + CB/LB 反向跳转 + 版本页入站 + 旧 redirect 清理 + PageSkeleton 覆盖 18 页 |
...
9. 📋 语义缓存评估（向量相似度复用，另立项）；高可用地基（多节点/Redis）于 v3.1.x 后评估
