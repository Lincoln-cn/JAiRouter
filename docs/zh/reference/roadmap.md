...
> **文档版本**: 2.0.0
> **最后更新**: 2026-09-19
> **Git 标签**: v3.2.0-pending（代码已在 master）
> **作者**: Lincoln
<!-- /版本信息 -->
...
### 🚧 v3.2.0（功能已合入 master，发布收口中）

**状态**: 功能代码经 PR #60 合入 master（2026-09-19）；版本号 / 产品文档 / 标签待收口
...
| v3.2.0 | 2026-09-19 | **已合 master（PR #60）**：PII 脱敏管理（控制台 + `/api/config/sanitization/**` + `sanitizeForStorage`）+ 配额监控限额/进度 UX + vitest/页面冒烟 TDD 地基 + 排除路径对齐 |
...
9. 🚧 v3.2.0 PII 治理与配额操作闭环 — **代码已于 2026-09-19 合入 master（PR #60）**；发布收口：版本号 bump、脱敏/控制台/管理 API 文档、全量回归、打标签
10. 📋 v3.2.1+ 候选：脱敏规则持久化版本化、配额 HOUR/MONTH 限额、记录级 FULL 加密策略 UI、试脱敏回归集
11. 📋 语义缓存评估（向量相似度复用，独立项目）；高可用地基（多节点/Redis）在 v3.2.x 后再评估
