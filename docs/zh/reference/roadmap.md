...
README 截图与内容更新（亮/暗主题 + 权限码 43→44 + 响应缓存 P1 特性）+ hallmark 管理台设计审计（只出报告不改码）+ 前端小修（权限码 44 同步等）。

### 🎯 v2.10.x - Web 管理台重构系列 (规划中)

前端功能已齐但未形成完整系统流程，按每版一主题拆分重构：
- **v2.10.0 前端地基**：hallmark 设计系统落地 + 公共组件统一 + 死代码清理 + 全局流程串联（Dashboard 治理链路 / 跨页导航）
...
5. 📋 v2.9.11 README/截图更新 + hallmark 管理台设计审计
6. 📋 v2.10.x Web 管理台重构系列（地基 / 配置接入 / 治理安全 / 双语版）→ v3.0
