...
响应缓存 P1：流式 SSE 拼接缓存 + 失效 API（CacheStore.delete + `DELETE /api/config/cache/response`）+ 服务级限流提前短路（命中免实例选择）。语义缓存仅评估另立项；Redis 实现后续路径评估。
...
3. ✅ v2.9.x 智能路由深化系列（故障转移/标签路由/响应缓存）
4. 📋 v2.9.10 响应缓存 P1（流式缓存 + 失效 API + 限流短路）
5. 📋 v2.10+ 高可用地基（多节点 + Redis 分布式 + 配置回滚）
6. 📋 语义缓存评估（向量相似度复用，单独立项）
