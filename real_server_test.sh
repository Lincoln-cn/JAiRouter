...
if [ $? -eq 0 ] && echo "$LIST_RESPONSE" | grep -q "success\|data"; then
    echo "✓ 成功获取实例列表"
...
# 检查创建是否成功
if echo "$CREATE_RESPONSE" | grep -q "success.*true\|添加实例成功\|更新实例成功"; then
    echo "✓ 测试实例创建成功"
else
    echo "✗ 测试实例创建失败"
...
if [ ! -z "$INSTANCE_DATA" ] && [ "$INSTANCE_DATA" != "" ]; then
    ...
    # 检查熔断器的其他配置
    CB_THRESHOLD=$(echo "$INSTANCE_DATA" | jq -r '.circuitBreaker.failureThreshold' 2>/dev/null)
    CB_TIMEOUT=$(echo "$INSTANCE_DATA" | jq -r '.circuitBreaker.timeout' 2>/dev/null)
    if [ "$CB_THRESHOLD" = "5" ] && [ "$CB_TIMEOUT" = "60000" ]; then
        echo "✓ 熔断器其他配置正确保存: 阈值=$CB_THRESHOLD, 超时=$CB_TIMEOUT"
    else
        echo "✗ 熔断器其他配置未正确保存: 阈值=$CB_THRESHOLD, 超时=$CB_TIMEOUT"
    fi
...
# 4. 更新实例配置
echo ""
echo "步骤 4: 更新实例配置 (禁用限流器，调整熔断器配置)"
...
# 检查更新是否成功
if echo "$UPDATE_RESPONSE" | grep -q "success.*true\|编辑成功\|更新实例成功"; then
    echo "✓ 实例配置更新成功"
else
    echo "✗ 实例配置更新失败"
...
if [ ! -z "$UPDATED_INSTANCE_DATA" ] && [ "$UPDATED_INSTANCE_DATA" != "" ]; then
    ...
    UPDATED_CB_TIMEOUT=$(echo "$UPDATED_INSTANCE_DATA" | jq -r '.circuitBreaker.timeout' 2>/dev/null)
    ...
    if [ "$UPDATED_CB_TIMEOUT" = "30000" ]; then
        echo "✓ 熔断器超时时间已正确更新: $UPDATED_CB_TIMEOUT"
    else
        echo "✗ 熔断器超时时间未正确更新: $UPDATED_CB_TIMEOUT"
    fi
...
if echo "$DELETE_RESPONSE" | grep -q "success.*true\|删除成功\|实例删除成功"; then
    echo "✓ 测试实例已成功删除"
else
    echo "⚠ 测试实例删除可能未成功"
